...
public class StatsMenu {
    ...
    public void openStatsMenu(Player player) {
        ...
        inv.setItem(10,item.createItem(Material.IRON_SWORD,"§a힘 §7" + PlayerDataStorage.map.get(player.getName()+"statsSTR"),
                "§f공격력이 §a" + String.format("%.2f",PlayerDataStorage.map.get(player.getName()+"statsSTR")*0.11) +"%§7➜"+ String.format("%.2f",(PlayerDataStorage.map.get(player.getName()+"statsSTR")+1)*0.11)+"% §f증가합니다",
                "",
                "§e클릭해서 1포인트 투자하기"));
        inv.setItem(12,item.createItem(Material.IRON_CHESTPLATE,"§a방어 §7" + PlayerDataStorage.map.get(player.getName()+"statsDEF"),
                "§f받은 피해의 §a" + String.format("%.2f", 100-(Math.pow(0.9992,PlayerDataStorage.map.get(player.getName()+"statsDEF"))*100 )) +"%§7➜"+String.format("%.2f", 100-(Math.pow(0.9992,PlayerDataStorage.map.get(player.getName()+"statsDEF")+1)*100 ))+"% §f를 방어합니다",
                "",
                "§e클릭해서 1포인트 투자하기"));
        inv.setItem(14,item.createItem(Material.FEATHER,"§a민첩 §7" + PlayerDataStorage.map.get(player.getName()+"statsDEX"),
                "§f이동 속도가 §a" + String.format("%.2f",PlayerDataStorage.map.get(player.getName()+"statsDEX")*0.07) +"%§7➜"+String.format("%.2f",(PlayerDataStorage.map.get(player.getName()+"statsDEX")+1)*0.07)+"% §f증가합니다",
                "",
                "§e클릭해서 1포인트 투자하기"));
        inv.setItem(16,item.createItem(Material.COOKED_BEEF,"§a생존 §7" + PlayerDataStorage.map.get(player.getName()+"statsSRV"),
                "§f배고픔이 §a" + String.format("%.2f", 100-(Math.pow(0.9974,PlayerDataStorage.map.get(player.getName()+"statsSRV"))*100 )) +"%§7➜"+String.format("%.2f", 100-(Math.pow(0.9974,PlayerDataStorage.map.get(player.getName()+"statsSRV")+1)*100 ))+"% §f확률로 닳지 않습니다",
                "",
                "§e클릭해서 1포인트 투자하기"));
