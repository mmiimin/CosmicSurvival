            ...
            if (accLevel > 0) {
                switch (PlayerDataStorage.map.get(player.getName() + "accessory" + slot)) {
                    ...
                    */
                    ...
                    case 11 -> {
                        ...
                        if (hitResult != null) {
                            ...
                            if (target instanceof LivingEntity) {
                                double dDistance = player.getLocation().distance(target.getLocation());
                                double damage = accLevel * 4.0 + 4;
                                if (dDistance>40) { damage*=0.1; }
                                else if (dDistance>35) { damage*=0.2; }
                                else if (dDistance>30) { damage*=0.35; }
                                else if (dDistance>25) { damage*=0.5; }
                                else if (dDistance>20) { damage*=0.7; }
                                else if (dDistance>15) { damage*=0.9; }
                    ...
                    /*case 12 -> {
                        ...
                        if (hitResult != null) {
                            ...
                            if (target instanceof LivingEntity) {
                                ...
                                target.getWorld().spawnParticle(Particle.GLOW, target.getLocation(), 5, 0.0, 1.0, 0.0, 0.2);
                            }
                        }
                        Vector vector = new Vector(end.getX() - start.getX(), end.getY() - start.getY(), end.getZ() - start.getZ());
                        int count = 30;
                    ...
                    /*case 13 -> {
                        player.getWorld().playSound(player.getLocation(), Sound.BLOCK_STONE_BUTTON_CLICK_OFF, 1.0F, 1.0F);
                        player.getWorld().playSound(player.getLocation(), Sound.BLOCK_STONE_BUTTON_CLICK_ON, 1.0F, 1.0F);
                        Location start = player.getEyeLocation();
                        Vector direction = start.getDirection();
                        start = start.clone().add(direction.clone().multiply(2));
                        int distance = 15;
                        cooldown = (50 - 8 * accLevel) * 20;
                        Location end = start.clone().add(direction.clone().multiply(distance));
                        RayTraceResult hitResult = player.getWorld().rayTrace(start, direction, distance, FluidCollisionMode.NEVER, true, 0.15, null);
                        if (hitResult != null) {
                            end = hitResult.getHitPosition().toLocation(player.getWorld());
                        ...
                        Location l = end;
                        plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                            player.getWorld().playSound(player.getLocation(), Sound.ENTITY_GHAST_SHOOT, 1.0F, 0.5F);
                            double r = 6;
                    ...
                    /*case 14 -> {
                        player.getWorld().playSound(player.getLocation(), Sound.ENTITY_ZOMBIE_VILLAGER_CURE, 1.0F, 2F);
                        ...
                        cooldown = (28 - 3 * accLevel) * 20;
                        AtomicReference<RayTraceResult> hitResult = new AtomicReference<>(player.getWorld().rayTrace(start, direction, distance, FluidCollisionMode.NEVER, true, 0.3, null));
                        AtomicReference<Location> end = new AtomicReference<>(start.clone().add(direction.clone().multiply(distance)));
                        AtomicReference<Vector> vector = new AtomicReference<>(new Vector(end.get().getX() - start.getX(), end.get().getY() - start.getY(), end.get().getZ() - start.getZ()));
                        int count = 2000;
                        for (int i = 0; i < count; i++) {
                            player.getWorld().spawnParticle(Particle.ASH, start.getX() + vector.get().getX() * 0.003 * i, start.getY() + vector.get().getY() * 0.003 * i, start.getZ() + vector.get().getZ() * 0.003 * i, 10, 0.02, 0.02, 0.02, 0.0);
                        }

                        for (int i = 0; i < 20; i++) {
                            Location finalStart = start;
                            plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                                player.getWorld().playSound(player.getLocation(), Sound.ENTITY_LIGHTNING_BOLT_IMPACT, 1.0F, 0.6F);
                                hitResult.set(player.getWorld().rayTrace(finalStart, direction, distance, FluidCollisionMode.NEVER, true, 0.3, null));
                                if (hitResult.get() != null) {
                                    end.set(hitResult.get().getHitPosition().toLocation(player.getWorld()));
                                    Entity target = hitResult.get().getHitEntity();
                                    if (target instanceof LivingEntity) {
                                        double damage = accLevel * 25.0 + 25;
                                        ((LivingEntity) target).setNoDamageTicks(0);
                                        ((LivingEntity) target).damage(damage, player);
                                        target.getWorld().spawnParticle(Particle.LAVA, target.getLocation(), 50, 0.0, 0.0, 0.0, 0.5);
                                        player.getWorld().playSound(player.getLocation(), Sound.ENTITY_BLAZE_HURT, 1.5F, 0.78F);
                                    }
                                }
                                for (int izi = 0; izi < count; izi++) {
                                    player.getWorld().spawnParticle(Particle.DRIP_LAVA, finalStart.getX() + vector.get().getX() * 0.003 * izi, finalStart.getY() + vector.get().getY() * 0.003 * izi, finalStart.getZ() + vector.get().getZ() * 0.003 * izi, 2, 0.02, 0.02, 0.02, 0.0);
                                }
